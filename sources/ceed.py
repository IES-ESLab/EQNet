...
def generate_phase_labels(
    ...
    Returns:
        ...
        - event_center: (nt,) - event center label
        - event_time: (nt,) - event time regression target
        - event_mask: (nt,) - mask for event
    """
    nt = sample.nt
    ...
    event_mask = np.zeros(nt, dtype=np.float32)
...
class CEEDDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        return {
            ...
            "phase_mask": torch.from_numpy(labels["phase_mask"][np.newaxis, :]).float(),
            ...
            "event_center": torch.from_numpy(labels["event_center"][np.newaxis, :]).float(),
            "event_time": torch.from_numpy(labels["event_time"][np.newaxis, :]).float(),
            "event_mask": torch.from_numpy(labels["event_mask"][np.newaxis, :]).float(),
        }
...
class CEEDIterableDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[dict[str, torch.Tensor]]:
        ...
        for i, record in enumerate(hf_dataset):
            ...
            # Yield when shuffle buffer is full
...
if __name__ == "__main__":
    ...
    ax.fill_between(range(len(labels["event_mask"])), labels["event_mask"] * 0.5, alpha=0.2)
    ax.set_ylabel("Event")
